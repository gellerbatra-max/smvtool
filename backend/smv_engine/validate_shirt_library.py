...
1. MACHINE-TIME-ONLY crosscheck (component-level, exact product match):
   ...
   recomputes it directly from seam_geometry.json's own seam/cycle records
   (bypassing shirt_library.py's handling-step wrapping entirely, so this is
   a geometry/machine-time regression guard, not an end-to-end check of
   shirt_library.py's operation assembly) and reports whether the two agree.
