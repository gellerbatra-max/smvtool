...
2. **Machine-time-in-minutes vs. the stored `machine_time_crosscheck`
   field: does NOT match, and the cause is UNRESOLVED.** Recomputing with
   this bundle's current `machine_time.py`/`effective_spm.py` gives ~4.47
   min total machine time (blended seam model + cycle time) against a
   stored 4.966 min, with the cycle-time component alone off by a factor of
   ~1.86x. An earlier draft of this report claimed the gap was explained by
   ...
   *current* code did not reproduce the stored figure either. The actual
   ...
   is reported as an **open, unresolved discrepancy** that should be
