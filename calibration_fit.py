...
def fit_operator_gamma(tax: "ht.Taxonomy", mcat: "mt.MachineCatalog", p_fitted: "es.MotionParams",
    ...
    already-fit scope-1(+2) model held fixed. Falls back to a plain
    factory-mean / operator-mean shrinkage estimator if statsmodels is
    unavailable or the mixed model fails to converge (flagged in the
    returned notes-string), so this scope never hard-fails the pipeline.
    """
    records = []
    for r in rows:
